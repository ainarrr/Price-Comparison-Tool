...
price_tool_frame["Unit Price"] = (price_tool_frame["Price"] / price_tool_frame[f"Amount ({converted_unit})"]).round(2)


# Currency formatting (using my currency function)
for var_item in ['Price', 'Unit Price']:
...
# Table Output
if item_type in ['q', 'quantity']:
    table_string = tabulate(price_tool_frame[['Item', amount_label, 'Price', 'Unit Price']],
                            headers='keys', tablefmt='psql', showindex=False)
else:
    table_string = tabulate(price_tool_frame[['Item', amount_label, f"Amount ({converted_unit})", 'Price', 'Unit Price']],
                            headers='keys', tablefmt='psql', showindex=False)
